...
export class HSBType {
  ...
  public toRGB(): number[] {
    ...
    const hInt = Math.floor(Number(h.multiply(new BigDecimal(5)).divide(new BigDecimal(3), 6).getValue()));
    const f = Number(h.multiply(new BigDecimal(5)).divide(new BigDecimal(3), 6).round(4).getValue()) % 1;
    const a = Number(
      v
  ...
  public toRGBBytes() {
    ...
    return [this.convertPercentToByte(red), this.convertPercentToByte(green), this.convertPercentToByte(blue)];
  }
